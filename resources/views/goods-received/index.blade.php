    ...
    <div class="space-y-6" x-data="goodsReceivedManager()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                <div class="flex items-center">
                    <div
                        class="w-12 h-12 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-xl flex items-center justify-center">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                <div class="flex items-center">
                    <div
                        class="w-12 h-12 bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-xl flex items-center justify-center">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <form method="GET" action="{{ route('goods-received.index') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div id="customDateRange" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4"
                        style="display: none;">
                        <div>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <a href="{{ route('goods-received.index', array_merge(request()->query(), ['sort' => 'receive_number', 'direction' => $sortField == 'receive_number' && $sortDirection == 'asc' ? 'desc' : 'asc'])) }}"
                                    ...
                                    @if ($sortField == 'receive_number')
                                        <i
                                            class="fas fa-sort-{{ $sortDirection == 'asc' ? 'up' : 'down' }} text-green-500"></i>
                                    @else
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-sm text-gray-500">
                                        {{ optional($gr->purchaseOrder)?->po_date?->format('d/m/Y') }}
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    {{-- <div class="text-xs text-gray-500">
                                        <div class="flex items-center space-x-2">
                                            <span class="text-blue-600">Stok:
                                                {{ number_format($summaryInfo['total_to_stock'])  ?? ''}}</span>
                                            <span class="text-green-600">Siap:
                                                {{ number_format($summaryInfo['total_to_ready']) }}</span>
                                        </div>
                                    </div> --}}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        ...
                                        <span
                                            class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $gr->status == 'complete' ? 'bg-green-400' : 'bg-yellow-400' }}"></span>
                                        {{ $statusInfo['text'] }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <button
                                            @click="showReceiptModal('{{ $gr->gr_id }}', '{{ addslashes($gr->receive_number) }}')"
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-8 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-gray-500 mb-4">Belum ada penerimaan barang yang tercatat dalam
                                            sistem.</p>
                                        <a href="{{ route('goods-received.create') }}"
        ...
        <!-- Receipt Modal -->
        <div x-show="receiptModal.show" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" @click.self="hideReceiptModal()"
            @keydown.escape.window="hideReceiptModal()"
            class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" style="display: none;">
            <div x-show="receiptModal.show" x-transition:enter="transition ease-out duration-300 transform"
                x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
                x-transition:leave="transition ease-in duration-200 transform"
                x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
                class="bg-white rounded-2xl shadow-2xl max-w-md w-full">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 text-center mb-6">
                        Cetak tanda terima untuk GR <span x-text="receiptModal.grNumber"
                            class="font-semibold text-gray-900"></span>?
                        Dokumen ini akan digunakan sebagai bukti penerimaan barang.
                    </p>

                    <div class="flex flex-col sm:flex-row gap-3">
                        <button type="button" @click="hideReceiptModal()"
                            class="flex-1 px-4 py-3 bg-gray-500 text-white rounded-xl hover:bg-gray-600 transition-all duration-200 flex items-center justify-center space-x-2">
                            <i class="fas fa-times"></i>
