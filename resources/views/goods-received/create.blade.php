    ...
    <div class="space-y-6" x-data="goodsReceivedCreate()">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center space-x-4">
                <div
                    class="w-16 h-16 bg-gradient-to-br from-green-600 to-green-700 rounded-2xl flex items-center justify-center">
                ...
                <div class="xl:col-span-3 space-y-6">
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <select id="po_id" name="po_id" x-model="selectedPOId" @change="onPOChange()"
                                        class="w-full py-3 px-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        <div x-show="!selectedPOId && !loadingPO" class="p-8 text-center">
                            ...
                            <p class="text-gray-500">Pilih PO terlebih dahulu untuk menampilkan items yang dapat diterima
                            </p>
                        </div>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    <template x-for="(item, index) in availableItems" :key="item.item_id">
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm font-medium text-gray-900"
                                                    x-text="formatNumber(item.remaining_quantity)"></div>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="space-y-2">
                                                    <input type="number" :name="`items[${index}][quantity_received]`"
                                                        ...
                                                        :max="item.remaining_quantity" step="1" min="0"
                                                        placeholder="0"
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="space-y-2">
                                                    ...
                                                    <button type="button" @click="openSerialNumberModal(index)"
                                                        :disabled="!item.quantity_received || item.quantity_received <= 0"
                                                        :class="item.quantity_received > 0 ?
                                                            'bg-blue-600 hover:bg-blue-700 text-white' :
                                                            'bg-gray-300 text-gray-500 cursor-not-allowed'"
                                                        class="px-3 py-2 rounded-lg text-sm transition-all">
                                                        <i class="fas fa-barcode mr-1"></i>
                                                        <span x-show="item.quantity_received > 0"
                                                            x-text="`Input SN (${item.quantity_received})`"></span>
                                                        <span
                                                            x-show="!item.quantity_received || item.quantity_received <= 0">Input
                                                            SN</span>
                                                    </button>

                                                    <!-- Serial number status -->
                                                    <div x-show="item.quantity_received > 0" class="text-xs">
                                                        <div x-show="getSerialNumberInputCount(index) === parseInt(item.quantity_received)"
                                                            class="text-green-600">
                                                            <i class="fas fa-check-circle mr-1"></i>
                                                            <span
                                                                x-text="`${getSerialNumberInputCount(index)} SN siap`"></span>
                                                        </div>
                                                        <div x-show="getSerialNumberInputCount(index) !== parseInt(item.quantity_received)"
                                                            class="text-orange-600">
                                                            <i class="fas fa-exclamation-triangle mr-1"></i>
                                                            <span
                                                                x-text="`${getSerialNumberInputCount(index)}/${item.quantity_received} SN`"></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                ...
                                <template x-for="(item, index) in availableItems" :key="'info-' + item.item_id">
                                    <div x-show="item.quantity_received > 0" class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                <label class="block text-xs font-medium text-gray-700 mb-1">Batch
                                                    Number</label>
                                            ...
                                            <div>
                                                <label class="block text-xs font-medium text-gray-700 mb-1">Expiry
                                                    Date</label>
                        ...
                        <!-- Items Summary -->
                        <div x-show="availableItems.length > 0 && !loadingPO"
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div class="text-center">
                                    <div class="text-lg font-bold text-purple-600" x-text="formatCurrency(totalValue)">
                                    </div>
                ...
                <div class="space-y-6">
                            ...
                            <div class="border-t pt-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-lg font-bold text-green-600"
                                        x-text="formatCurrency(totalValue)"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        <div class="p-6 space-y-4">
                            ...
                            <button type="button" @click="generateAllSerialNumbers()"
        ...
        <!-- Serial Number Modal -->
        <div x-show="showSerialModal" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div x-show="currentItemForSerial">
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 mb-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <h4 class="font-medium text-gray-900" x-text="currentItemForSerial?.item_name">
                                        </h4>
                                        <p class="text-sm text-gray-500" x-text="currentItemForSerial?.item_code"></p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-lg font-bold text-blue-600"
                                            x-text="currentItemForSerial?.quantity_received + ' units'"></p>
                            ...
                            <div class="max-h-96 overflow-y-auto">
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <template x-for="(serial, serialIndex) in currentSerialNumbers"
                                        :key="serialIndex">
                                        <div class="space-y-2">
                                            <label class="block text-sm font-medium text-gray-700"
                                                x-text="`Unit ${serialIndex + 1}`"></label>
                                            <div class="relative">
                                                <input type="text" x-model="serial.value"
                                                ...
                                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
                                                    <i x-show="serial.validating"
                                                        class="fas fa-spinner fa-spin text-gray-400"></i>
                                                    <i x-show="!serial.validating && serial.valid && serial.value"
                                                        class="fas fa-check text-green-500"></i>
                                                    <i x-show="!serial.validating && serial.error"
                                                        class="fas fa-times text-red-500"></i>
                                                </div>
                                            </div>

                                            <!-- Error Message -->
                                            <div x-show="serial.error" class="text-xs text-red-600"
                                                x-text="serial.errorMessage"></div>
                                        </div>
                                    </template>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 flex flex-col sm:flex-row gap-3">
                                <button type="button" @click="generateSerialNumbersForItem()"
                                    class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                                    <i class="fas fa-magic mr-2"></i>
                                    Auto Generate Semua
                                </button>

                                <button type="button" @click="clearSerialNumbersForItem()"
                                    class="flex-1 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors">
                                    <i class="fas fa-eraser mr-2"></i>
                                    Clear Semua
                                </button>

                                <button type="button" @click="saveSerialNumbers()" :disabled="!canSaveSerialNumbers"
    ...
    <script>
        function goodsReceivedCreate() {
            return {
                ...
                // Computed properties
                get totalReceived() {
                    return this.availableItems.reduce((total, item) => total + (parseInt(item.quantity_received) || 0),
                        0);
                },
                    ...
                    try {
                        ...
                        if (data.po && data.details) {
                            ...
                            this.showNotification(
                                `Berhasil memuat ${data.details.length} items dari PO ${data.po.po_number}`,
                                'success');
                        } else {
                ...
                // ✅ FIXED: validateSerialNumber method
                async validateSerialNumber(serialIndex) {
                    ...
                    try {
                        ...
                        const clientValidation = this.validateF1FormatClient(serialNumber);
                        if (!clientValidation.valid) {
                            serialObj.valid = false;
                            serialObj.error = true;
                            serialObj.errorMessage = clientValidation.message;
                            serialObj.validating = false;
                            return;
                        }

                        // ✅ FIX: POST request with proper JSON body
                        const response = await fetch('/api/goods-received/validate-serial-number', {
                            ...
                            body: JSON.stringify({ // ✅ JSON body for POST
                        ...
                        serialObj.valid = data.valid;
                        serialObj.validating = false;
                        serialObj.validatedAt = new Date().toISOString();

                        if (!data.valid) {
                            serialObj.error = true;
                            serialObj.errorMessage = data.message;

                            // Enhanced error info for duplicates
                            if (data.error_type === 'duplicate' && data.existing_info) {
                                serialObj.errorMessage += ` (Digunakan oleh: ${data.existing_info.item_code})`;
                            }
                        } else {
                            serialObj.errorMessage = '';

                            // Show format info for valid F1 numbers
                            if (data.format_info) {
                                console.log(`✅ Valid F1 Serial: ${serialNumber}`, data.format_info);
                            }
                        }
                        ...
                        serialObj.valid = false;
                        serialObj.error = true;
                        serialObj.errorMessage = 'Error validasi: ' + error.message;
                        serialObj.validating = false;
                    }
                },
                ...
                async generateSerialNumbersForItem() {
                    ...
                    try {
                        // Show loading state
                        this.showNotification('Generating F1 serial numbers...', 'info');

                        // ✅ FIX: Use URLSearchParams for GET request - NO BODY!
                        const params = new URLSearchParams({
                            item_id: this.currentItemForSerial.item_id,
                            quantity: this.currentItemForSerial.quantity_received
                        });

                        const response = await fetch(`/api/goods-received/serial-number-template?${params}`, {
                        ...
                        if (data.success) {
                            // Apply generated serial numbers
                            data.data.serial_numbers.forEach((sn, index) => {
                                if (this.currentSerialNumbers[index]) {
                                    ...
                                    this.currentSerialNumbers[index].validatedAt = new Date().toISOString();
                                }
                            });

                            // Log generation info
                            console.log('🎯 F1 Serial Numbers Generated:', {
                                format: data.data.format,
                                pattern: data.data.pattern,
                                total: data.data.total_generated,
                                serials: data.data.serial_numbers,
                                item: data.data.item_info
                            });

                            this.showNotification(
                                `Generated ${data.data.total_generated} F1 serial numbers (${data.data.format})`,
                                'success'
                            );

                            // Auto-validate all generated numbers
                            await this.validateAllSerialNumbers();

                        } else {
                            this.showNotification('Gagal generate F1 serial numbers: ' + data.message, 'error');
                        }
                    } catch (error) {
                        console.error('Generate F1 serial numbers error:', error);
                        this.showNotification('Error generating F1 serial numbers: ' + error.message, 'error');
                    }
                },


                async validateAllSerialNumbers() {
                    const serialNumbers = this.currentSerialNumbers
                        .filter(sn => sn.value.trim())
                        .map(sn => sn.value.trim());

                    if (serialNumbers.length === 0) {
                        this.showNotification('Tidak ada serial numbers untuk divalidasi', 'warning');
                        return;
                    }

                    try {
                        this.showNotification('Validating all serial numbers...', 'info');

                        // ✅ FIX: POST request with proper JSON body
                        const response = await fetch('/api/goods-received/bulk-validate-serial-numbers', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json', // ✅ JSON content type
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                        ...
                        if (data.success) {
                            // Apply bulk validation results
                            Object.entries(data.data.results).forEach(([index, result]) => {
                                const serialIndex = parseInt(index);
                                if (this.currentSerialNumbers[serialIndex]) {
                                    this.currentSerialNumbers[serialIndex].valid = result.valid;
                                    this.currentSerialNumbers[serialIndex].error = !result.valid;
                                    this.currentSerialNumbers[serialIndex].errorMessage = result.valid ? '' :
                                        result.message;
                                    this.currentSerialNumbers[serialIndex].validatedAt = new Date()
                                        .toISOString();
                                }
                            });
                            ...
                            if (summary.invalid > 0) {
                                this.showNotification(
                                    `Validation completed: ${summary.valid} valid, ${summary.invalid} invalid`,
                                    'warning'
                                );
                            } else {
                                this.showNotification(
                                    `All ${summary.total} serial numbers are valid!`,
                                    'success'
                                );
                            }
                            ...
                            console.log('📊 Bulk Validation Results:', {
                                summary: summary,
                                all_valid: data.data.all_valid,
                                details: data.data.results
                            });

                        } else {
                            this.showNotification('Bulk validation error: ' + data.message, 'error');
                        }

                    } catch (error) {
                        console.error('Bulk validation error:', error);
                        this.showNotification('Error during bulk validation: ' + error.message, 'error');
                    }
                },
                ...
                saveSerialNumbers() {
                    ...
                    this.showNotification(
                        `${serialValues.length} serial numbers disimpan untuk ${this.currentItemForSerial.item_name}`,
                        'success');
                        ...
                        if (quantity > 0) {
                            try {
                                const response = await fetch(
                                    `/api/goods-received/serial-number-template?item_id=${item.item_id}&quantity=${quantity}`, {
                                        headers: {
                ...
                async getSerialNumberStats() {
                    try {
                        // ✅ FIX: Simple GET request without body
                        const response = await fetch('/api/goods-received/serial-number-stats', {
                            method: 'GET',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                                    'content'),
                        ...
                        if (data.success) {
                            console.log('📈 Serial Number Statistics:', data.data);

                            this.showNotification(
                                `Stats: ${data.data.total_serial_numbers} total, ${data.data.f1_format_count} F1 format`,
                                'info'
                            );
                ...
                handleSerialNumberInput(event, index) {
                    const input = event.target;
                    let value = input.value.toUpperCase().trim();

                    // Auto-format F1 prefix
                    if (value.length > 0 && !value.startsWith('F1')) {
                        if (value.startsWith('F')) {
                            value = 'F1' + value.substring(1);
                        } else {
                            value = 'F1' + value;
                        }
                    }
                    ...
                    // Filter out confusing characters in real-time
                    const confusingChars = ['0', 'O', 'I', '1', 'l', 'B', '8', '6', 'G', '5', 'S'];
                    let cleanValue = value;
                    let removedChars = [];

                    confusingChars.forEach(char => {
                        if (cleanValue.includes(char)) {
                    ...
                    if (removedChars.length > 0) {
                        this.showNotification(`Karakter yang membingungkan dihapus: ${removedChars.join(', ')}`, 'info');
                    }

                    // Update value
                    this.currentSerialNumbers[index].value = cleanValue;
                    input.value = cleanValue;

                    // Auto-validate on input (debounced)
                    clearTimeout(this.validationTimeout);
                    this.validationTimeout = setTimeout(() => {
                        if (cleanValue.length === 8) { // Only validate complete F1 serials
                            this.validateSerialNumber(index);
                        }
                    }, 500);
                },
                    ...
                    try {
                        ...
                        this.availableItems.forEach((item, index) => {
                            ...
                            if (qtyReceived > 0) {
                                ...
                                // Optional fields
                                const batchNumber = document.querySelector(
                                    `input[name="items[${index}][batch_number]"]`)?.value;
                                const expiryDate = document.querySelector(
                                    `input[name="items[${index}][expiry_date]"]`)?.value;
                                const itemNotes = document.querySelector(`input[name="items[${index}][notes]"]`)
                                    ?.value;
