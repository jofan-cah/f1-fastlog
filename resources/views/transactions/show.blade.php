...
<div class="space-y-6" x-data="transactionShow()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            ...
            @endphp
            <div class="w-16 h-16 bg-gradient-to-br {{ $typeInfo['gradient'] ?? 'from-blue-600 to-blue-700' }} rounded-2xl flex items-center justify-center">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="p-3 bg-gray-50 rounded-xl border">
                                <span class="text-sm text-gray-900">{{ $transaction->transaction_date->format('d M Y H:i') }}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="p-3 bg-gray-50 rounded-xl border">
                                <span class="text-sm font-mono text-gray-900">{{ $transaction->reference_id }}</span>
                            </div>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <div class="p-3 bg-gray-50 rounded-xl border">
                                <span class="text-sm font-semibold text-gray-900">{{ $transaction->quantity }} item(s)</span>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="flex flex-col md:flex-row items-start md:items-center gap-2">
                                            @if($detail->hasStatusChanged())
                                                <div class="flex items-center space-x-2">
                                                    <span class="px-2 py-1 rounded-full text-xs font-medium {{ $statusChange['before']['class'] }}">
                                                        {{ $statusChange['before']['text'] }}
                                                    </span>
                                                    <i class="{{ $changeImpact['icon'] }} {{ $changeImpact['class'] }}"></i>
                                                    <span class="px-2 py-1 rounded-full text-xs font-medium {{ $statusChange['after']['class'] }}">
                                ...
                                <div class="relative pb-8">
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                <p class="text-sm text-gray-500">Transaksi dibuat oleh <span class="font-medium text-gray-900">{{ $transaction->createdBy->full_name ?? 'Unknown' }}</span></p>
                                            </div>
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                <p class="text-sm text-gray-500">
                                                    Transaksi {{ $transaction->status === 'approved' ? 'disetujui' : 'ditolak' }} oleh
                                                    <span class="font-medium text-gray-900">{{ $transaction->approvedBy->full_name ?? 'Unknown' }}</span>
                                                </p>
                                            </div>
                                            <div class="text-right text-sm whitespace-nowrap text-gray-500">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div class="text-center mb-4">
                        ...
                        <h4 class="font-semibold text-gray-900">{{ $transaction->item->item_name ?? 'Multiple Items' }}</h4>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-medium">{{ $transaction->item->category->category_name ?? 'N/A' }}</span>
                        </div>
                        @endif
                        ...
                        @if($transaction->approved_date)
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600">{{ $transaction->status === 'approved' ? 'Disetujui' : 'Ditolak' }}</span>
                            <span class="text-sm font-medium">{{ $transaction->approved_date->format('d/m/Y') }}</span>
                        </div>
                        @endif
                    </div>
                </div>
    ...
    <div x-show="showApprovalModal"
         x-cloak
         @keydown.escape.window="showApprovalModal = false"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="showApprovalModal"
                 ...
                 aria-hidden="true"
                 @click="showApprovalModal = false"></div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Yakin ingin approve transaksi <strong>{{ $transaction->transaction_number }}</strong>?
    ...
    <div x-show="showRejectionModal"
         x-cloak
         @keydown.escape.window="showRejectionModal = false"
         class="fixed inset-0 z-50 overflow-y-auto"
         aria-labelledby="modal-title"
         role="dialog"
         aria-modal="true">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Yakin ingin reject transaksi <strong>{{ $transaction->transaction_number }}</strong>?
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <textarea id="rejection_reason" name="reason" rows="3"
                                            ...
                                            placeholder="Jelaskan alasan rejection..."
                                            required></textarea>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.addEventListener('keydown', function(e) {
            ...
            if (e.ctrlKey && e.key === 'a' && '{{ $transaction->status }}' === 'pending' && {{ \App\Models\Transaction::canUserApprove() ? 'true' : 'false' }}) {
                e.preventDefault();
                const transactionShow = window.Alpine.findClosest(document.body, x => x.__x_$data?.showApprovalModal !== undefined)?.__x_$data;
                if (transactionShow) {
            ...
            if (e.ctrlKey && e.key === 'e' && '{{ $transaction->status }}' === 'pending' && {{ $transaction->created_by === auth()->id() ? 'true' : 'false' }}) {
                e.preventDefault();
