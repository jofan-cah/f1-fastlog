    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center space-x-4">
                ...
                <div class="w-16 h-16 bg-gradient-to-br from-blue-600 to-purple-700 rounded-2xl flex items-center justify-center">
            ...
            <div class="flex items-center space-x-3">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                    {{ $transaction->status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                       ($transaction->status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }}">
                    <i class="fas {{ $transaction->status === 'pending' ? 'fa-clock' :
                                   ($transaction->status === 'approved' ? 'fa-check-circle' : 'fa-times-circle') }} mr-2"></i>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                        {{ $transaction->transaction_type === 'OUT' ? 'bg-blue-100 text-blue-800' :
                           ($transaction->transaction_type === 'IN' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800') }}">
                        <i class="fas {{ $transaction->transaction_type === 'OUT' ? 'fa-arrow-up' :
                                        ($transaction->transaction_type === 'IN' ? 'fa-arrow-down' : 'fa-wrench') }} mr-1"></i>
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $transaction->transaction_date->format('d M Y, H:i') }}</span>
                </div>

                <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Created by:</span>
                    <span class="text-sm font-medium text-gray-900">{{ $transaction->createdBy->full_name ?? 'Unknown' }}</span>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <p class="text-sm font-medium text-gray-900">{{ $transaction->item->item_name ?? 'Unknown Item' }}</p>
                </div>
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center space-x-3 mb-2">
                                        ...
                                        <div>
                                            <h4 class="font-medium text-gray-900">{{ $detail->itemDetail->serial_number ?? 'No Serial Number' }}</h4>
                                            <p class="text-sm text-gray-600">{{ $detail->itemDetail->item->item_name ?? 'Unknown Item' }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                                        <div>
                                            ...
                                            <div class="mt-1">
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                                    {{ $detail->status_before === 'available' ? 'bg-green-100 text-green-800' :
                                                       ($detail->status_before === 'used' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800') }}">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-700 mt-1">{{ $detail->itemDetail->location ?? 'Unknown' }}</p>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="space-y-4">
                <div class="flex items-start space-x-3">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center
                        ...
                        <i class="fas {{ $transaction->status === 'approved' ? 'fa-check text-green-600' : 'fa-times text-red-600' }}"></i>
                    </div>
                    <div class="flex-1">
                        <div class="flex items-center space-x-2">
                            <span class="font-medium text-gray-900">{{ $transaction->approvedBy->full_name ?? 'Unknown' }}</span>
                            <span class="text-sm text-gray-500">{{ $transaction->status === 'approved' ? 'approved' : 'rejected' }} this transaction</span>
                        </div>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-4">
                                    You are about to reject transaction <strong>{{ $transaction->transaction_number }}</strong>.
