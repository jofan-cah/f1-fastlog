    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        <button @click="showQuickAdjustModal('add')"
                        ...
                        <button @click="showQuickAdjustModal('reduce')"
                                class="w-full px-4 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl hover:from-red-700 hover:to-red-800 transition-all duration-200 flex items-center space-x-2">
                            <i class="fas fa-minus"></i>
                            <span>Kurangi Stok</span>
                        </button>

                        <button @click="showQuickAdjustModal('return')"
                        ...
                        <a href="{{ route('items.show', $stock->item->item_id) }}"
                ...
                <div class="p-6">
                    <div class="space-y-4 text-sm">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-medium text-blue-600">{{ number_format($stock->quantity_available) }} {{ $stock->item->unit }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">Stok Terpakai:</span>
                            <span class="font-medium text-yellow-600">{{ number_format($stock->quantity_used) }} {{ $stock->item->unit }}</span>
                        </div>
                        <hr class="border-gray-200">
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600 font-medium">Total Stok:</span>
                            <span class="font-bold text-green-600">{{ number_format($stock->total_quantity) }} {{ $stock->item->unit }}</span>
                        </div>
                        <hr class="border-gray-200">
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">Minimum Stock:</span>
                            <span class="font-medium text-gray-900">{{ number_format($stock->item->min_stock) }} {{ $stock->item->unit }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">Persentase Tersedia:</span>
                            <span class="font-medium text-indigo-600">{{ $stock->getStockPercentage() }}%</span>
                        </div>

                        @if($stock->quantity_available > $stock->item->min_stock)
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600">Buffer Stock:</span>
                                <span class="font-medium text-green-600">{{ number_format($stock->quantity_available - $stock->item->min_stock) }} {{ $stock->item->unit }}</span>
                            </div>
                        @else
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600">Kekurangan:</span>
                                <span class="font-medium text-red-600">{{ number_format($stock->item->min_stock - $stock->quantity_available) }} {{ $stock->item->unit }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900 font-mono">{{ $stock->stock_id }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Item ID:</span>
                            <span class="text-gray-900 font-mono">{{ $stock->item->item_id }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Dibuat:</span>
                            <span class="text-gray-900">{{ $stock->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Diupdate:</span>
                            <span class="text-gray-900">{{ $stock->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                        @if($stock->last_updated)
                            <div class="flex justify-between">
                                <span class="text-gray-600">Stok Update:</span>
                                <span class="text-gray-900">{{ $stock->last_updated->format('d/m/Y H:i') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
